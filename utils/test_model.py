...
def test_model(opt, cnn, log=None):
    ...
    if opt.rank==0:
        print_log('\n',log,arrow=False)
        print_log('[INFO=>] Setup_params -- Train:',log,arrow=False)
    ...
    if opt.test_id_loc:
        ...
        if opt.rank==0:
            ...
            print_log(f'[INFO==>] Test on {len(imgList)}/{len(all_imgs)} samples in {imgs_path}', log,arrow=False)
            print_log(imgList,log)
    else:
        imgList = [x[2:-4] for x in os.listdir(os.path.join(imgs_path,'noise')) if 'mat' in x and 'im' in x]
        imgList.sort()
        if opt.rank==0:
            print_log('[INFO==>] Test {} samples in {}'.format(len(imgList),imgs_path), log,arrow=False)
    ...
    if label_existence:
        ...
        if opt.rank == 0:
            print_log('[INFO==>] Label exists',log,arrow=False)
    ...
    cnn.eval()
    print_log('\n',log,arrow=False)
    print_log('[INFO=>] Start test',log,arrow=False)
        ...
        for im_ind, (im_tensor, target, target_2d, fileid) in enumerate(data_generator):
            ...
            if opt.rank==0:
                if label_existence:
                    ...
                    print_log(f'Test sample {im_ind}/{len(imgList)} Index {fileid[0]} found {nemitters:d}/{len(xyz_gt)} emitters',log)
                else:
                    print_log(f'Test sample {im_ind}/{len(imgList)} Index {fileid[0]} found {nemitters:d} emitters',log)
