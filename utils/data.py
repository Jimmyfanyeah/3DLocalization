...
class ImagesDataset_v2(Dataset):
    ...
    def __getitem__(self, index):
        ID = str(self.list_IDs[index])

        im_name = os.path.join(self.root_dir,'noise','im' + ID + '.mat')
        ...
        try:
            gtimg_name = os.path.join(self.root_dir,'clean','im' + ID + '.mat')
...
def batch_xyz_to_boolean_grid(xyz_np, opt):
    ...
    # print(f'{np.max(xg)} {np.max(xyz_np[:,:,0])}')
    # print(f'{np.max(yg)}, {np.max(xyz_np[:,:,1])}')
    # print(np.max(zg))
