...
def learn_localization(rank,world_size,opt):
    ...
    if opt.train_or_test == 'train':
       # split dataset to train, validation 9:1
        ...
        # log files
        if rank == 0:
            log = open(os.path.join(opt.save_path, '{}_log.txt'.format(time.strftime('%m-%d-%H-%M-%S'))), 'w')
            logger = Logger(os.path.join(opt.save_path, '{}_tensorboard'.format(time.strftime('%m-%d-%H-%M-%S'))))

            print_log('[INFO=>] setup_params:',log,arrow=False)
            for key,value in opt._get_kwargs():
                if not key == 'partition':
                    print_log('{}: {}'.format(key,value),log)
            print_log(f'[INFO=>] Dataset: Train {len(train_IDs)} Val {len(val_IDs)}',log, arrow=False)

        device = torch.device('cuda')
        torch.backends.cudnn.benchmark = True

        if opt.rank==0:
            # save setup parameters in result folder
        ...
        if opt.rank == 0:
            # print_log(model, log)
            print_log("[INFO=>] Number of parameters in model: {}\n".format(sum(param.numel() for param in model.parameters())),log,arrow=False)

        # if resume_training, continue from a checkpoint
        if opt.resume:
            print_log('[INFO=>] Resume...',log,arrow=False)
        ...
        if opt.postpro:
            ...
            print_log(f"[INFO==>] Apply post-process with thresh {opt.postpro_params['thresh']} and radius {opt.postpro_params['radius']}...",log)
        ...
        if rank == 0:
            ...
            print_log('[INFO==>] setup params -- Test:',log,arrow=False)
            for key,value in opt._get_kwargs():
                if not key == 'partition':
                    print_log('{}: {}'.format(key,value),log)
...
if __name__ == '__main__':
    ...
    # phase
    parser.add_argument('--train_or_test', type=str, default=None, help='train or test')
    ...
    parser.add_argument('--zmin', type=int, default=None, help='min zeta')
    ...
    parser.add_argument('--data_path', type=str, default=None, help='path for train and val data')
    parser.add_argument('--save_path', type=str, default=None, help='path for save models and results')
    parser.add_argument('--name_time', type=str, default=None)
    opt = parser.parse_args()
    opt.zmin = opt.zmin if opt.zmin else -opt.zmax
